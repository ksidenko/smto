...
class CAdvancedArbehavior extends CActiveRecordBehavior
{
		...
		foreach($this->owner->relations() as $key => $relation)
		{
			if($relation['0'] == CActiveRecord::MANY_MANY) // ['0'] equals relationType
			{
                                ...
                                if(isset($this->owner->$key))
				{
                                        ...
                                        #echo CVarDumper::dump($attribute[0],2);exit;
					Yii::trace('found MANY_MANY relation '.$key.' in '.get_class($this->owner),'system.db.ar.CActiveRecord');

                                        // NOT SET BY POST VARS
                                        if(isset($attribute[0]) && is_object($attribute[0]))
					{
                                            #Yii::trace("isobj");exit;
                                            $this->executeManyManyEntry($this->makeManyManyDeleteCommand(
							$relation[2],
							$this->owner->{$this->owner->tableSchema->primaryKey}));
                                        }
                                        // ??
                                        else if(is_numeric($this->owner->$key))
					{
						$this->executeManyManyEntry($this->makeManyManyDeleteCommand(
							$relation[2],
							$this->owner->{$this->owner->tableSchema->primaryKey}));
					...
					{
        #                                        echo CVarDumper::dump($this->owner->$key, 3);exit;
						$this->executeManyManyEntry($this->makeManyManyDeleteCommand(
							$relation[2],
							$this->owner->{$this->owner->tableSchema->primaryKey}));
	...
	public function executeManyManyEntry($query) {
                #Yii::trace('Execute SQL: '.$query,'system.db.CDbConnection');
		$this->owner->dbConnection->createCommand($query)->execute();
	}
