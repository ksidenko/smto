 ...
 *
 * $Id: Image_ImageMagick_Driver.php 450 2010-02-24 08:55:47Z schmunk $
 *
...
class Image_ImageMagick_Driver extends Image_Driver {
	...
	public function process($image, $actions, $dir, $file, $render = FALSE)
	{
		...
		if ($status = $this->execute($actions))
		{
			...
			if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' -quality '.$quality.'% '.$this->cmd_image.' -colorspace RGB +profile "*" '.$this->new_image))
			{
				$this->errors[] = $error;
			}
			else
	...
	public function resize($prop)
	{
		...
		if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' -geometry '.$dim.' '.$this->cmd_image.' '.$this->cmd_image))
		{
			$this->errors[] = $error;
			return FALSE;
		}
